    ...
    <PageReveal title={t('menu.title')} className="bg-[var(--ivory-50)] min-h-screen pt-32 pb-24">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col lg:flex-row gap-12 lg:gap-24 relative">
        ...
        <div className="lg:w-48 flex-shrink-0">
          <nav className="sticky top-28 hidden lg:flex flex-col gap-4 border-l border-[var(--ivory-200)] pl-6">
            <p className="font-condensed uppercase tracking-widest text-xs text-[var(--muted)] mb-2">Sections</p>
            {menuCategories.map((cat) => (
              <button
                key={cat.id}
                onClick={() => scrollToCategory(cat.id)}
                className="text-left font-serif text-lg text-[var(--espresso-900)] hover:text-[var(--terracotta)] transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus"
              >
                {l(cat.name)}
              </button>
            ))}
          ...
          {/* Mobile Jump Links */}
          <nav className="lg:hidden flex overflow-x-auto pb-4 gap-6 scrollbar-hide sticky top-20 bg-[var(--ivory-50)]/90 backdrop-blur-sm z-30 pt-4 -mx-4 px-4 border-b border-[var(--ivory-200)]">
             {menuCategories.map((cat) => (
              <button
                key={cat.id}
                onClick={() => scrollToCategory(cat.id)}
                className="whitespace-nowrap font-condensed tracking-widest text-xs uppercase text-[var(--espresso-900)] hover:text-[var(--terracotta)] transition-colors"
              >
                {l(cat.name)}
              </button>
            ))}
        ...
        <div className="flex-1">
          {menuCategories.map((cat, index) => (
            <motion.div 
              ...
              className={cn("mb-24 scroll-mt-36", index === menuCategories.length - 1 ? 'mb-0' : '')}
            >
